...
def main():
    ...
    while sum(sample_wise_save_list) <  expanded_number:  
        ...
        sample_wise_save_list, class_save_list, selected_class_save_list = dataset_expansion(trainloader, model, GIF_model,  DDIM_sampler, epoch, use_cuda, class_names, text_classifier, class_save_list, selected_class_save_list, sample_wise_save_list, expanded_number_per_sample, number_per_split, batch_size)
        epoch += 1
...
def dataset_expansion(train_loader, model,GIF_model,  DDIM_sampler, epoch, use_cuda, class_names, text_classifier, class_save_list, selected_class_save_list, sample_wise_save_list, expanded_number_per_sample, number_per_split, batch_size):
    ...
    for batch_idx, (original_inputs, augmented_inputs, targets, index) in enumerate(train_loader):
        with torch.no_grad():
            ...
            # domain_lsit = ["an image of", "a real-world photo of", "a cartoon image of",  "an oil painting of", "a sketch of"]
            # adjective_lsit = ["", "colorful", "stylized", "high-contrast", "low-contrast", "posterized", "solarized", "sheared", "bright", "dark"]   
            
            domain_lsit = [
                "a realistic photo of a real-life person with",
                "a real-life portrait of a realistic person showing",
                "a high-quality image of a real-life person expressing",
                "a detailed photograph of a real-life person displaying",
                "a lifelike picture of a real-life person depicting"
            ]

            adjective_lsit = [
                "genuine", "authentic", "sincere", "unfeigned", "heartfelt",
                "expressive", "convincing", "compelling", "believable", "realistic",
                "natural-looking", "true-to-life", "faithful", "accurate", "credible"
            ]
        ...
        channel_noise = Variable(torch.rand([expanded_number_per_sample_each_epoch,channel_noise_dim, 1,1]),  requires_grad=True).cuda() 
        channel_noise_bias =  Variable(torch.zeros([expanded_number_per_sample_each_epoch,channel_noise_dim, 1,1]).data.normal_(0, 1),  requires_grad=True).cuda()
